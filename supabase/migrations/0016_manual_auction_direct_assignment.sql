-- ─────────────────────────────────────────────────────────────────────────
-- 0016: Manual auction direct player-owner assignment.
--
-- SKPL teams have no separate non-playing "owner" — one of the team's own
-- players logs in and bids on the team's behalf during a Manual/Live
-- auction. That player can't go through the ordinary auction pool (they'd
-- be biddable by other teams, or the person driving bids would be bidding
-- on themselves), so the admin now assigns them straight onto the roster
-- when starting a Manual auction, at an admin-entered price — analogous
-- to a retention mechanically, but a fresh admin decision made at start
-- time, not pulled from season_retentions.
--
-- Extends start_manual_auction with p_direct_assignments (jsonb array of
-- {team_id, player_id, price}), realized in the same slot as retentions —
-- before pool-seeding, so the assigned player is automatically excluded
-- from the pool with no change needed to that query. Exactly one direct
-- assignment is allowed per team, and a player who is also retained this
-- season is rejected (retention and direct assignment are mutually
-- exclusive ways onto a roster). AI-mode start_auction is untouched.
-- ─────────────────────────────────────────────────────────────────────────
...
create function start_manual_auction(
  p_season_id uuid,
