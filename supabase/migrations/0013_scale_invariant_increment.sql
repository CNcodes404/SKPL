-- Fixes advance_auction_bid's bid increment, which was a fixed tiny amount
-- (5-50) chosen for a toy 2-3 digit currency scale. Against real prices in
-- the lakhs (7 digits), that increment was negligible — any team that could
-- afford the current bid could always afford +50, so nothing about it ever
-- meaningfully separated bidders, and combined with a tight min=max squad
-- size (which pushes the auction into critical mode almost immediately),
-- every player ended up selling at exactly base price. The increment is now
-- a percentage of the current bid, so it scales with whatever currency
-- magnitude the league actually uses.
