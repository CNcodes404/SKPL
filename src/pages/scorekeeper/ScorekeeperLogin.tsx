...
import { signInWithPassword } from '@/services/scorekeeper'

export default function ScorekeeperLogin() {
  ...
  if (!authLoading && session) {
    ...
    return <Navigate to={from?.pathname ?? '/scorekeeper'} replace />
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-skpl-gradient p-4">
      <div className="w-full max-w-sm rounded-xl bg-white p-8 shadow-elevated">
        <div className="mb-6 flex flex-col items-center gap-2 text-center">
          ...
          <h1 className="font-display text-xl font-extrabold text-primary-900">Scorekeeper Login</h1>
          <p className="text-sm text-muted-foreground">Sign in to track matches live as a spectator.</p>
        </div>
